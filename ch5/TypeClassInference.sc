...
object StrParser{
  ...
  implicit def ParseSeq[T](implicit p: StrParser[T]) = new StrParser[Seq[T]]{
    def parse(s: String) = splitExpression(s).map(p.parse)
  }
...
def splitExpression(s: String): Seq[String] = {
  ...
  for (i <- Range(1, s.length - 1)) {
    s(i) match {
      case '[' => openBrackets += 1
      case ']' => openBrackets -= 1
      case ',' => if (openBrackets == 0) indices += i
      case _ => // do nothing 
    }
  }
  val allIndices: Seq[Int] = Seq(0) ++ indices ++ Seq(s.length - 1)
  for (i <- Range(1, allIndices.length).toList) yield s.substring(allIndices(i - 1) + 1, allIndices(i))
}
